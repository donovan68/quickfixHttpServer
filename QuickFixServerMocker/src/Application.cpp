...
void Application::onLogon( const FIX::SessionID& sessionID ) { _sessionIds.insert(sessionID.toString()); _sessionMap.insert(std::pair<std::string, FIX::Session*>(sessionID.toString(), _acceptor->getSession(sessionID))); }
void Application::onLogout( const FIX::SessionID& sessionID ) { _sessionMap.erase(sessionID.toString()); }
