...
public:
    typedef boost::shared_ptr<FIX::ThreadedSocketAcceptor> AcceptorSmartPtr;
    typedef std::map<std::string, FIX::Session*> SessionMap;
    typedef SessionMap::const_iterator SessionMapConstItr;
    typedef SessionMap::iterator SessionMapItr;
    
    typedef std::set<std::string> SessionIdSet;
private:
    AcceptorSmartPtr _acceptor;
    SessionMap _sessionMap;
    SessionIdSet _sessionIds;
public:

    Application() : m_orderID(0), m_execID(0) {
    }

    void setAcceptor(boost::shared_ptr<FIX::ThreadedSocketAcceptor> acceptor){
        _acceptor = acceptor;
    }
    
    FIX::Session* getSession(const std::string& sessionId){
        SessionMapConstItr it = _sessionMap.find(sessionId);
    ...
    AcceptorSmartPtr getAcceptor() {
        return _acceptor;
    }
    
    SessionIdSet getSessionIDSet(){
        return _sessionIds;
    }
